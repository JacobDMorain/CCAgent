  ...
  return [
    {
      ...
      content: [
        ...
        "{roleTeam}",
        "",
        "Provider outputs must be grouped by role using headings exactly like `## Role: <role name>`.",
        "If a role finds no actionable issues, include that role heading and say no actionable issues were found.",
        "",
        "Lead with findings ordered by severity. For each issue include:",
        "- title",
        "- evidence from the file",
    ...
    },
    {
      ...
      content: [
        ...
        "",
        "The review packet may be role grouped: provider review findings can be grouped by `### Role:` sections inside each provider section.",
        "Evaluate each role finding independently. A finding is not automatically correct just because a role or provider raised it.",
        "",
    ...
    },
    {
      ...
      content: [
        ...
        "",
        "Provider outputs must be grouped by role using headings exactly like `## Role: <role name>`.",
        "如果某个角色没有发现可执行问题，也必须保留该角色标题并说明没有可执行问题。",
        "",
    ...
    },
    {
      ...
      content: [
        ...
        "Failed providers: {failedProviders}",
        "",
        "Review packet 可能是 role grouped：每个 provider 区块内部可能按 `### Role:` 角色分段组织评审意见。",
        "请独立判断每个角色提出的意见，不要因为某个角色或 provider 提出就机械采纳。",
        "",
