...
Use `docs/local-secrets.md` for the operator-supplied `GLM_API_KEY` and `DEEPSEEK_API_KEY` values. Prefer current-session PowerShell environment variables; if a local file is used, keep it in `.env.local` and never commit it.
