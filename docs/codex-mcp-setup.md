...
The command backs up the existing Codex config and adds a `ccagent` MCP block that points at the built MCP server entrypoint. The MCP server will read the daemon token from the local CCAgent secret store when it runs under the same Windows account as the daemon.
...
Do not put provider API keys in the MCP configuration. Provider keys are saved through the daemon secret store.
...
| API key missing | The provider exists but its `apiKeyRef` has no stored secret. | Save the API key through the provider form or `POST /providers/:id/secret`. |
